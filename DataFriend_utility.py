...
def update_vals(c,k):
    ...
    s=[]# Список индексов для срезов
    d=[]# Список индексов для срезов со смещением на -1
    ...
    for elements in df_vals['variable']:# Находит элементы в столбце с названием переменных.
        if elements!='Nin':# Если это,что-то отличное от Nin
            e=''# будет содержать индекс для полученного значения
            e=df_vals.loc[df_vals['variable']==elements].index[0] #получение индекса значения
            s.append(e)# добавляет индекс в список
    for i in s:# в цикле создается второй список с индексами для среза, все значения уменьшаются на 1
        if i<=1:#Условие для добавления в список 0 и 1,если она есть
            d.append(i)
        else:
            i=i-1#уменьшение всех значений на -1
            d.append(i)#
