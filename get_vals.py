def get_vals(c,read_csv,DataFrame):
    ...
    df = read_csv ( '{}.csv'.format (c[:-4]) , encoding = 'utf-8' , sep = ';', low_memory=False)#открываем файл в УТФ-8 используя разделитель ";", отключено ограничение на использование памяти для более точного определения типа столбца
    ...
    k = DataFrame ( columns= ['variable', 'index', 'values', 'Df_expr', 'Df_metr', 'restrict_W'] )#создание пустого датафрейма,из которого создается валс
